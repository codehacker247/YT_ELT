  ...
  try:
    if schema == 'staging':
      ...
      cur.execute(
        ...
        VALUES (%(video_id)s, %(title)s, %(published_At)s, %(duration)s, %(viewCount)s, %(likes_count)s, %(commentCount)s)
        """,row
      )
...
def update_rows(cur,conn,schema,row):
  try:
    ...
    logger.info(f"Row updated successfully in {schema}.{table} for Video_ID: {row[video_id]}")
